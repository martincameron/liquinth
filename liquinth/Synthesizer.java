...
public class Synthesizer {
	...
	/*
		Please do not modify out_buf between invocations, as
		the downsampling filter requires the samples left in
		the buffer by the previous invocation.
	*/
	public synchronized void get_audio( int[] out_buf, int length ) {
		int idx, input_ep1;
		int cutoff, alevel;
		/* Copy end samples from previous invocation.*/
		input_ep1 = length * OVERSAMPLE;
		for( idx = 0; idx < FIR_COEFFS; idx++ ) {
			out_buf[ idx ] = out_buf[ input_ep1 + idx ];
		}		
		/* Clear mix buffer.*/
		input_ep1 += FIR_COEFFS;
		for( idx = FIR_COEFFS; idx < input_ep1; idx++ ) {
			out_buf[ idx ] = 0;
		}
		/* Get audio from voices. */
		for( idx = 0; idx < NUM_VOICES; idx++ ) {
			voices[ idx ].get_audio( out_buf, FIR_COEFFS, length * OVERSAMPLE );
		}
